...
def test_clean_pharmacy_date_parsing():
    ...
    assert pd.to_datetime(cleaned_data['nad_imputed'], errors='coerce').notnull().all(), "Dates not parsed correctly"
...
def test_clean_visits_date_parsing():
    ...
    # Assert dates are parsed correctly
    assert pd.to_datetime(cleaned_data['visitdate'], errors='coerce').notnull().all(), "visitdate not parsed correctly"
    assert pd.to_datetime(cleaned_data['nextappointmentdate'], errors='coerce').notnull().all(), "nextappointmentdate not parsed correctly"
...
def test_parse_long_date():
    ...
    # Test valid date
    assert parse_long_date("2023-01-01T12:00:00") == datetime(2023, 1, 1).date(), "Valid date not parsed correctly"
