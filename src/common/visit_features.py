...
def clean_visittype(visit_type_vec):
    ...
    visit_type_vec = np.where(
        visit_type_vec.str.contains("unscheduled", na=False), 1, 0
    )
...
def clean_differentiatedcare(diff_care_vec):
    ...
    diff_care_vec = np.where(
        diff_care_vec.str.contains("community art distribution"),
...
def clean_pregnancy(df):
    ...
    df["pregnancy_age"] = np.where(
        (df["age"] >= 15) & (df["age"] <= 49) & (df["sex"] == 0), 1, 0
    )

    # Prepare conditions
    yes_mask = (df["pregnancy_age"] == 1) & (
        df["pregnant"].str.contains("yes", case=False, na=False)
    )
    no_mask = (df["pregnancy_age"] == 1) & (
        df["pregnant"].str.contains("no", case=False, na=False)
    )

    # Use np.select to assign values
    df["pregnant"] = np.select([yes_mask, no_mask], [1, 0], default=None)

    # finally, create binary flag to indicate if pregnant status is missing or not there because it's not relevant
    df["pregnant_missing"] = np.where(
        df["pregnant"].isnull() & (df["sex"] == 0) & (df["pregnancy_age"] == 1), 1, 0
    )
...
def clean_breastfeeding(df):
    # Prepare conditions
    yes_mask = (df["pregnancy_age"] == 1) & (
        df["breastfeeding"].str.contains("yes", case=False, na=False)
    )
    no_mask = (df["pregnancy_age"] == 1) & (
        df["breastfeeding"].str.contains("no", case=False, na=False)
    )

    # Use np.select to assign values
    df["breastfeeding"] = np.select([yes_mask, no_mask], [1, 0], default=None)

    # Create binary flag to indicate if breastfeeding status is missing or not relevant
    df["breastfeeding_missing"] = np.where(
        df["breastfeeding"].isnull() & (df["sex"] == 0) & (df["pregnancy_age"] == 1),
        1,
        0,
    )
...
def clean_bmi(df):
    ...
    df["bmi"] = np.where(
        (df["height_valid"] == 1) & (df["weight_valid"] == 1) & (df["age"] >= 15),
...
def regimen_switch_group(group):
    ...
    for _, this_date in enumerate(visitdates):
        mask = (visitdates <= this_date) & (
            visitdates >= this_date - pd.Timedelta(days=365)
        )
        prev_regimens = regimens[mask].dropna().unique()
        switch_counts.append(len(prev_regimens))

    group["regimen_switch"] = [
        None if x == 0 else (0 if x == 1 else 1) for x in switch_counts
    ]
    return group
