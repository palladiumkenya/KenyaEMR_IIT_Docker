...
def refresh_model(pipeline = False, targets_df = None, targets_aws = None, refresh_date = str):
    ...
    # encoded dataset
    dtrain = encode_xgboost(df, start_date = after, end_date = refresh_date - pd.DateOffset(months=1))
    dval = encode_xgboost(df, start_date = refresh_date - pd.DateOffset(months=1), end_date = refresh_date)
